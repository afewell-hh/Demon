...
#[tokio::test]
async fn given_config_with_secrets_when_dispatch_echo_then_secret_resolved() {
    let temp_dir = TempDir::new().unwrap();
    let contracts_dir = temp_dir.path().join("contracts");
    let config_dir = temp_dir.path().join("config");
    ...
    fs::write(config_dir.join("echo.json"), config_with_secrets).unwrap();
    ...
    let config_manager = ConfigManager::with_dirs(contracts_dir, config_dir);
    let secret_provider = EnvFileSecretProvider::with_secrets_file(secrets_file);
    ...
    match result {
        ...
        Err(e) => {
            // If NATS is not available, we might get a connection error
            // In that case, we verify the error is related to NATS, not config/secrets
            let error_msg = e.to_string();
            assert!(
                error_msg.contains("Failed to connect to NATS")
                    || error_msg.contains("connection refused")
...
#[tokio::test]
async fn given_config_with_missing_secret_when_dispatch_echo_then_secret_resolution_failure() {
    let temp_dir = TempDir::new().unwrap();
    let contracts_dir = temp_dir.path().join("contracts");
    let config_dir = temp_dir.path().join("config");
    ...
    // Create config file with secret references that don't exist
    let config_with_secrets = r#"{
